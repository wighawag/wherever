...
- **Frames.** `session_created` carries `folderMissing?: boolean` (the flag), and `folder_missing { sessionId, cwd }` states it authoritatively with the ABSOLUTE missing path so the UI can name what to restore (later work extends that frame with any running restore job; remote CANDIDATES deliberately do NOT ride on it, see below, because resolving them can shell out to a provider CLI and this frame is on the session-load path). The shared client mirrors both into `WhereverState.folderMissing` (`FolderMissingInfo`), and the web dashboard replaces the composer with a notice naming the path.
- **Older clients degrade safely**: they simply do not render the notice. The protection is the SERVER's refusal -- a `message` from a client whose loaded session has a missing folder is refused by PATH (`client.folderMissingCwd`), before the attachment check that would otherwise drop it silently.
- **Partly built:** the restore ENGINE exists (`server/src/restore-jobs.ts`, below) but nothing is wired to it yet, so a folder-missing session is still locked with no remedy offered in-app, which is strictly better than an agent running against a directory that is not there. See `docs/adr/0009` and `docs/adr/0010`.

**Vocabulary, pinned:** the STATE is `folder missing` (frame `folder_missing`, flag `folderMissing`, `WhereverState.folderMissing`); `restore` names only the ACTIONS that cure it (cloning or creating the folder). Do not fork either term: `restore` already has an unrelated in-codebase meaning for re-materialising queued steers and saved drafts after a reload (`restoredFromQueue`), so a missing folder is never "unrestored" and a queued steer is never "missing".
