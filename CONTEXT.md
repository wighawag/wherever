...
- **The browser learns the token from the page URL's HASH FRAGMENT (`web/src/lib/core/token-adoption.ts`).** `https://host/#token=SECRET` is the documented single-link form: on boot the token is parsed out with `URLSearchParams` over the substring after `#`, MERGED into the existing `wherever-config` localStorage entry (token field only, so host/port/`hideThinking`/`beepDefault` survive) and then stripped from the address bar with `history.replaceState` (never `pushState`, which would leave the secret in the back-button history). Everything downstream is unchanged: `client/src/client.ts` already puts `config.token` on the `/ws` URL, and the server already accepts it on the upgrade and on the API routes, so the gap this closes was purely how the browser FIRST learns the token (previously Connection Settings only, which is why `?token=` rendered the page but left the WebSocket unauthenticated). The hash is preferred over a query parameter because a fragment is never sent to the server: no access log, no `Referer`, no proxy request log. `?token=` is still accepted as a fallback for links already handed out; both forms are scrubbed. A blank/whitespace token is scrubbed but NOT stored (storing `''` would turn a malformed link into a logout), while a non-blank one is stored VERBATIM, because the server compares it byte-for-byte and a token with surrounding whitespace is a supported install. The key is matched case-insensitively for a reason that is specific to this app: the fragment is ALSO the session-id channel (`+page.svelte` feeds `location.hash` into `switchSession`), so an unrecognised `#TOKEN=SECRET` would be sent to the server as a session id and echoed back into the DOM as `Session with ID "<secret>" not found`. Adoption is idempotent (`adoptTokenFromUrlOnce`) and invoked from `getConfig()` in `web/src/lib/wherever.ts` plus `session-store.ts` module scope, because `getConfig()` runs at MODULE scope (the `beepDefaultStore` initialiser) and `session-store` reads the token straight out of localStorage: a `+layout.svelte` hook would be too late for both.
- **The scrub needs TWO passes, and the second one is not optional (`scrubTokenFromUrl` + `afterNavigate` in `web/src/routes/+layout.svelte`).** The dashboard boots in CSR mode (the generated `index.html` calls `kit.start(app, element)` with no hydrate payload), so SvelteKit's `start()` reads `location.href` and runs an initial `enter` navigation with it. That navigation is what imports the route modules, i.e. where adoption runs, so the first scrub happens while the navigation is still in flight; when it commits, the router stamps the history entry with the href it captured BEFORE the import and the token REAPPEARS in the address bar. Measured in a real browser: the scrub lands, then ~12ms later the router's own `history.replaceState` undoes it. `afterNavigate` fires after that commit, which is why the layout calls `scrubTokenFromUrl()` there. No node-environment test can observe this (no router), so the suite pins the wiring with a source-level tripwire on `+layout.svelte`; the acceptance evidence is a real browser run.
...
### Server sessions run the pi extension lifecycle (bind on create, shutdown before dispose)

`session-pool.ts` builds a live agent in exactly two places, both `createAgentSession()` calls: `loadSession` (open an existing transcript) and `createNewSession` (a fresh one). `createAgentSession()` only LOADS extensions; in the pi SDK it is `AgentSession.bindExtensions(...)` that emits `session_start` and then `resources_discover` (extension-contributed skills, prompts, themes), and only `AgentSessionRuntime.dispose()` emits `session_shutdown` (a bare `AgentSession.dispose()` does not). Wherever owns bare AgentSessions, so it runs the lifecycle itself, through `server/src/extension-lifecycle.ts`:

- **Bind on create.** Both paths call `bindServerSessionExtensions()` right after `createAgentSession()` and BEFORE the session is put in the pool or handed to a client. If binding throws, the session is shut down and disposed and the create reports an error. The bind is NOT time-bounded (cancelling a half-started extension set is worse than waiting), but a watchdog logs to the journal when `session_start` is still running after `EXTENSION_BIND_WARN_AFTER_MS` (15s), since a hung bind otherwise shows up only as a session that never opens. Without this, extensions that initialise on `session_start` never start: the pi-mcp-adapter answered every `mcp(...)` call with "MCP not initialized" in web sessions.
- **The bindings are deliberate** (full rationale in the code comment on `serverExtensionBindings`): no `uiContext` (so `ctx.hasUI` is false and extensions take their non-interactive path, since the web UI has no channel for extension dialogs), `mode: 'print'` (the SDK's headless mode, the one consistent with `hasUI === false`), an `onError` that logs every extension error to stderr (the journal), and minimal `commandContextActions`: only `waitForIdle` acts, while `newSession` / `fork` / `switchSession` / `navigateTree` answer `{ cancelled: true }` because sessions are owned by the pool and swapped only through the web protocol. `reload` is refused too (logged, no-op, as it was before bindings existed): `AgentSession.reload()` calls pi-ai's `resetApiProviders()`, which is process-wide, so one session's reload would drop the custom API providers other live sessions' extensions registered. `shutdownHandler` is unset so an extension cannot stop the multi-session server.
- **Shutdown before dispose.** `shutdownAndDisposeAgentSession()` is the ONLY way a server session is disposed: it first aborts any in-flight turn synchronously (`agent.abort()`, mirroring `dispose()`'s own first step; otherwise a live turn would keep running tools and appending to the transcript during the shutdown window, recreating a just-deleted file or racing a CLI that took over), then emits `session_shutdown` with `reason: "quit"` to the session's current extension runner, awaits it bounded by `EXTENSION_SHUTDOWN_TIMEOUT_MS` (5s; a hung extension is logged and the session is disposed anyway), then calls `dispose()`. It never rejects. Every teardown goes through it: `destroySession` (manual close, delete, idle eviction; the pool entry is dropped synchronously, and the method returns the teardown promise), `disposeAll` (server shutdown, all sessions in parallel so shutdown costs at most one timeout), and a CLI bridge taking over a live server session (the CLI entry replaces the server one in the pool first, then the teardown is awaited).
- **CLI-bridge sessions need nothing.** Their agent runs in a real `pi` process, which binds its own extensions; the server only holds a WebSocket for them.
- Covered by `server/test/extension-lifecycle.test.ts` (an inline recorder extension injected through `SessionPool`'s `extraExtensionFactories` option, the test-only seam next to the built-in conversation-mode extension; `extensionShutdownTimeoutMs` / `extensionBindWarnAfterMs` shorten the bounds in tests).
- **Known gap:** `destroySession` drops the pool entry before the teardown finishes, so reopening the same file immediately can start a new agent while the old one's `session_shutdown` is still running. An extension holding a fixed resource (an OAuth callback port) could collide.
...
- **Folder EXISTENCE is cached per folder, never per session** (`annotateFolderExistence` in `server/src/session-pool.ts`). The listing reports `missing` per folder (see below), and a `stat` per SESSION would put a syscall storm back into the very pass the two caches above keep free of IO. So the check is one `stat` per DISTINCT folder path (the folders the scan just grouped), cached for 10s and counted by `getFolderExistenceCheckCount()` for the tests, and invalidated when a restore job settles. The 10s window is the other half of the freshness story: a folder that reappears by means wherever knows nothing about (cloned by hand in a terminal, a mount coming back) loses its mark within it, with no invalidation at all.
...
### The session browser marks folders that are not on this machine (`missing`)

The LISTING view of the same state: on a freshly migrated machine most folders are absent, and opening every conversation one at a time is otherwise the only way to find that out. `GET /sessions` therefore stamps each folder with `missing: true` when its cwd is not a directory on this machine, and `SessionBrowser.svelte` renders a small **Missing** chip beside the folder name.

- **It is a SECOND, ORTHOGONAL per-folder fact, not a second flavour of `readOnly`.** `readOnly` is a configured policy (`sessions.readOnly`), lives on its own page, and hides the create/delete controls; `missing` is a fact about the disk and is CURABLE by a restore. A folder can be either, both or neither, so the chip is its own marker (purple, uppercase, with a title naming the condition) and never the read-only treatment.
- **Named `missing`, not `folderMissing`.** It is the SAME pinned `folder missing` state, seen per folder rather than per session; the object it sits on IS a folder, exactly as `readOnly` drops the prefix there. The session-level flag on `session_created` keeps its `folderMissing` name because the object there is a session.
- **The chip says "Missing", not "not cloned".** CREATE is a first-class remedy for a folder that was never a clone, so "not cloned" would be wrong for it and would fork the pinned vocabulary of the state it marks.
- **The listing does NOT OR-in a running restore job**, unlike `session_load` (which must, because `git clone` makes the target directory in its first breath and a client loading mid-clone would otherwise be handed an agent on a half-cloned tree). A listing hands out no capability, and teaching the session pool about the job registry would fork the layering for a chip; so a folder briefly loses its mark while a clone runs, then keeps it if the clone fails (the settle invalidates the cache either way).
- **Invalidation subscribes to the registry DIRECTLY** (`observeRestoreForFolderExistence` in `server/src/index.ts`), a second subscription beside the panel's broadcast relay, per the registry's path-keyed observation seam. Every terminal state invalidates the cached answer (a cancelled clone REMOVES a directory it created); only a `done` job additionally calls `onSessionsUpdated()`, so an open dashboard refetches and the chip disappears with no manual refresh. The new-session clone passes no callback: the session it then creates broadcasts on its own.
...
- **It ends in a RELOAD** (`client.reloadSession()` -> a fresh `session_load` of the same file). Whether a session has a live agent is a load-time decision and the load that found the folder missing built none, so re-loading is the actual remedy, not a refresh hint. The NEW-SESSION path cannot do this (there is no session to reload until the clone lands) and continues into creation itself instead; see the next section and `docs/adr/0009`.

### The new-session clone is the SAME job, and it is the only clone left

Creating a session in a folder that does not exist can CLONE AN EXISTING REMOTE instead of creating one (the clone-or-create dialog, offered when `/check-remote-repo` says the repository the create WOULD provision already exists). That branch is the second entry point of `docs/adr/0009` and it runs through the same restore registry, so there is exactly ONE clone implementation in the codebase: the same recursive engine, the same URL allowlist and home guard, the same honest progress. The old synchronous `cloneRemoteRepo()` in `session-pool.ts` is DELETED -- it cloned non-recursively, reported nothing, and ran inside the create, so a large repository always lost the race with the client's 25s create watchdog.

- **Where it runs.** `cloneForNewSession()` in `server/src/index.ts` resolves the target the way `createNewSession` does (`resolveCreateCwd`), matches a `remoteRepoRules` entry with the SHARED `matchRemoteRepoRule`, probes it with `detectRemoteRepo`, and requests a clone job. `SessionPool.createNewSession` no longer takes `cloneRemote` at all: by the time it is called the folder is there. If no rule matches or the probe finds nothing, the clone is SKIPPED and an ordinary create follows, which is exactly what the synchronous path did.
- **The URL is resolved server-side**, unlike the panel's editable field: the create flow has no URL input, so a client-supplied URL would be a new authority nobody asked for. Both entry points are validated by the registry regardless.
- **The ending is the asymmetry.** A loaded session offers a Reload; here there is nothing to reload until the clone lands, so the server AWAITS the job and continues into `createNewSession` itself when it is `done`. A `failed` or `cancelled` job fails the create with the mapped cause on `session_error` (the create's own channel) and makes no session; the `restore_complete` frame carrying git's raw output is broadcast beside it.
- **Frames are the ordinary ones.** The requester gets `restore_started`; progress and completion arrive through the same derived path-matching broadcast, which reaches this client because the handler records the target as its `pendingCwd` before starting (a create has no session yet). Cancel needs nothing new either: the job is path-keyed and server-owned, so a plain `restore_cancel` settles it and thereby abandons the create.
- **The client adopts the job as the create's own.** `restore_started` arriving while `creatingSession` is true with no restore state sets `WhereverState.restore` with `forSessionCreate: true`, drops `creatingSession` and DISARMS the create watchdog -- a watched job is not something to time out. The dashboard then renders `RestoreProgress.svelte` (the display shared with the panel) in place of the blocking overlay. On `restore_complete` with `done` the client re-arms `creatingSession` for the last, ordinary step the server is already performing; on a failure it clears the restore and lets `session_error` speak.
- **`POST /session/new` takes the same branch** (its `cloneRemote` still works, now recursive), it simply has no progress channel and waits for the job.
- Leaving or switching sessions mid-clone stops the PAINTING, not the job: it is server-owned, so it finishes and the session it was for is created and shows up in the session list.
