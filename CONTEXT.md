...
- **Not yet built (deliberate):** the restore ACTIONS (clone/create + progress) are separate work; see `docs/adr/0009` and `docs/adr/0010`. Until they land, a folder-missing session is locked with no remedy offered in-app, which is strictly better than an agent running against a directory that is not there.
